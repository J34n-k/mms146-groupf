...
def play():
    while True:
        ...
        PLAY_PLAYER2 = Button(image=pygame.image.load("assets/Player Button.png"), pos=(640, 530), 
        ...
        for button in [PLAY_PLAYER1, PLAY_PLAYER2, PLAY_PLAYER3, PLAY_BACK_BUTTON]:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if PLAY_PLAYER1.check_for_input(PLAY_MOUSE_POS):
                    name_input(1)
                if PLAY_PLAYER2.check_for_input(PLAY_MOUSE_POS):
                    name_input(2)
...
def name_input(num_players):
    ...
    while True:
        ...
        for i in range(num_players):
            ...
            display_name = names[i] if names[i] else f"Player {i+1}"
            text_surface = input_font.render(display_name, True, color)
            text_rect = text_surface.get_rect(center=(640, 200 + i * 100))
            box_rect = text_rect.inflate(300, 20)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN:
                    ...
                    if len(names[active_index]) < 20:
                        names[active_index] += event.unicode
            if event.type == pygame.MOUSEBUTTONDOWN:
                for i in range(num_players):
                    text_surface = input_font.render(names[i] if names[i] else f"Player {i+1}", True, "white")
                    text_rect = text_surface.get_rect(center=(640, 200 + i * 100))
                    box_rect = text_rect.inflate(300, 20)
...
def show_question(question_obj):
    while True:
        SCREEN.blit(PLAY_BG, (0, 0))
        BG = pygame.image.load("assets/questions background.png")

        mouse_pos = pygame.mouse.get_pos()
        
        # Display the question text
        question_text_surface = get_font(30).render(question_obj.question, True, "White")
        question_text_rect = question_text_surface.get_rect(center=(640, 200))
        SCREEN.blit(question_text_surface, question_text_rect)

        # Back button to return to the board
        back_btn = Button(image=None, pos=(640, 650),
...
def game_board(names):
    while True:
        SCREEN.blit(PLAY_BG, (0, 0))
        mouse_pos = pygame.mouse.get_pos()

        x_start = 100
        font_cat = get_font(25)
        for category_name in CATEGORIES.keys():
            text_surface = font_cat.render(category_name, True, "White")
            text_rect = text_surface.get_rect(center=(x_start, 50))
            SCREEN.blit(text_surface, text_rect)
            x_start += 250
        ...
        for category_name, questions in CATEGORIES.items():
            y_start = 150
            for points, question_obj in questions.items():
                if not question_obj.answered:
                    button = Button(image=None, pos=(x_start, y_start),
                                    text_input=str(points), font=get_font(40),
                                    base_color="White", hovering_color="Yellow")
                    button.change_color(mouse_pos)
                    button.update(SCREEN)
                    all_buttons.append((button, category_name, points))
                else:
                    answered_button = Button(image=None, pos=(x_start, y_start),
                                             text_input="", font=get_font(40),
                                             base_color="#0d128a", hovering_color="#0d128a")
                    answered_button.update(SCREEN)
                y_start += 100
            x_start += 250
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                sys.exit()
            if event.type == pygame.MOUSEBUTTONDOWN:
                for button, cat, pts in all_buttons:
                    if button.check_for_input(mouse_pos):
                        selected_question = CATEGORIES[cat][pts]
                        show_question(selected_question)
                        # We must exit this inner loop after clicking the button
                        # to prevent the board from being redrawn
                        break
